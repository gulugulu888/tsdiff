...
def _load_custom_jsonl_dataset(
    ...
    if meta_file.exists():
        try:
            ...
            if meta_pred_len is not None:
                dataset_meta_prediction_length = meta_pred_len # This is for the Dataset object, model uses config's
            ...
            if actual_dataset_freq_from_meta != config_freq:
                print(f"Warning: Training config frequency ('{config_freq}') differs from dataset's metadata.json frequency ('{actual_dataset_freq_from_meta}'). "
                      f"The TSDiff model will be built using the config frequency: '{config_freq}'. "
            ...
            print(f"Warning: Could not correctly parse metadata.json from {meta_file}: {e}. Relying on training config values for dataset object.")
    else:
    ...
    # For the GluonTS TrainDatasets object, use the frequency specified in the training config,
    # as this is what the model architecture (lags, time features) will be based on.
    # The prediction_length for MetaData can also come from config, as it's primarily for the Dataset object's info.
    ...
    return TrainDatasets(
        ...
        train=JsonLinesFile(path=train_file, freq=config_freq), 
        test=JsonLinesFile(path=test_file, freq=config_freq),   
    )
